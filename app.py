...
import numpy as np
import pandas as pd
from lightfm_functions import get_clean_dataframes, get_recommendation, create_lightfm_dataset, train_lightfm_model
from plot_functions import create_full_interactions_df, plot_map, popular_books, show_book_cover
import pickle
...
def get_all_data():
    ...
    full_interactions_df = create_full_interactions_df(interactions_df, users_df, items_df)
    return interactions_df, full_interactions_df, items_df, users_df, dataset, user_ids_buffered, item_ids_buffered, interactions, weights, model


interactions_df, full_interactions_df, items_df, users_df, dataset, user_ids_buffered, item_ids_buffered, interactions, weights, model = get_all_data()


def project_description_tab():
    st.markdown("## Проект студентов группы МПИ-22-1 Александра Петрова и Рашида Мейланова")
    st.markdown("#### Ссылка на датасет https://www.kaggle.com/datasets/arashnic/book-recommendation-dataset/data")
...
def map_tab():
    ...
    agg_fun_option = st.selectbox('Choose aggregation function', 
                                  ['Number of ratings by Country', 'Number of users by Country', 'Mean rating by Country'],
                                   index=0)
    ...
    if agg_fun_option == 'Number of ratings by Country':
        ...
        fig = plot_map(full_interactions_df[['user_id', 'rating', 'Country Code']].drop_duplicates(subset=['user_id', 'Country Code'], keep='first'), 'count')
...
def popular_books_tab():
    ...
    with col1:
        age_filter = st.selectbox(
            'Choose age',
            ['Не использовать', '0-10','11-19','19-30','31-40','41-60','61+','Age_Unknown'],
            index=0,
    ...
    with col2:
        country_filter = st.selectbox(
            'Choose country',
            ['Не использовать'] + full_interactions_df['Country'].value_counts().index.values.tolist(),
            index=0,
    ...
    if st.button('Show most popular books', key='tab_popular_books_tab'):
        popularBooks = popular_books(full_interactions_df, n_top_filter, age_filter, country_filter)
        if type(popularBooks) == str:
            ...
            for ind in popularBooks.index:
                ...
                with col1_1:
                    ...
                    st.write(f"             Publication Year: {popularBooks.loc[ind, 'Publication Year']}")
                    st.write(f"             Publisher: {popularBooks.loc[ind, 'Publisher']}")
                    st.write(f"             Popularity: {popularBooks.loc[ind, 'Popularity']}")
                    st.write(f"             Average Rating: {popularBooks.loc[ind, 'AverageRatings']}")

                with col1_2:
                    st.image(show_book_cover(popularBooks.loc[ind, 'image_url']), use_column_width=True)
...
def recommendation_tab():
    ...
    with col3:
        task = st.selectbox('What to show',
    ...
    if st.button('Compute results', key='tab_recommendations'):
        ...
        already_rated_inds, recommended_inds = get_recommendation(model, dataset, interactions_df, option_user_id, item_ids_buffered, option_n_recs)
        ...
        if task == 'Recommendations':
            ...
            temp_rec = pd.merge(items_df, interactions_df[interactions_df['user_id'] == option_user_id][['item_id', 'rating']])
        ...
        for ind in temp_rec.index:
            ...
            with col1_1:
                ...
                st.write(f"             Publication Year: {temp_rec.loc[ind, 'Publication Year']}")
                st.write(f"             Publisher: {temp_rec.loc[ind, 'Publisher']}")

            with col1_2:
                st.image(show_book_cover(temp_rec.loc[ind, 'image_url']), use_column_width=True)
    ...
    with st.form('Fill your books and ratings'):
        for i in range(number_of_books_read):
            ...
            with book_col:
                ...
                book_read_id, author_temp = items_df[items_df['Title'] == book_read][['item_id', 'Author']].values[0]
        ...
        if submitted:
            user_temp_df = pd.DataFrame({'user_id': [new_user_id]*number_of_books_read, 'item_id': books_read_ids, 'rating': ratings_of_read_books})
            interactions_df_temp = pd.concat([interactions_df, user_temp_df])

            dataset_temp, user_ids_buffered_temp, item_ids_buffered_temp, interactions_temp, weights_temp = create_lightfm_dataset(interactions_df_temp)
            
            with st.spinner('Getting predictions...'):
                model_temp = train_lightfm_model(weights=weights_temp)

            already_rated_inds, recommended_inds = get_recommendation(model_temp, dataset_temp, interactions_df_temp, new_user_id, item_ids_buffered_temp, 10)
            temp_rec = items_df[items_df['item_id'].isin(recommended_inds)]
            st.markdown('## Recommendations')
            for ind in temp_rec.index:
                col1_1, col1_2 = st.columns([0.6, 0.4])
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Project Description", "Map plot", "The most popular books", "Recommendations", "New user recommendations"])
